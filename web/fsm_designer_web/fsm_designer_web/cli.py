...
Usage:
    ...
    fsm-designer [options] start
    fsm-designer [options] status
    fsm-designer [options] stop
    fsm-designer [options] kill
...
logger = logging.getLogger('fsm-designer.cli')

LOG_FILE = 'fsm-designer.log'
PID_FILE = 'fsm-designer.pid'
...
def main(args=None):
    ...
    if parsed_args['status']:
        ...
        if pid:
            ...
            if not p.is_running():
                logger.warning("fsm-designer not running")
                return 0
            else:
                logger.info("fsm-designer is running")
                return 0
        else:
            logger.warning("fsm-designer not running")
            return 1
